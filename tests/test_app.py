...
# Dados simulados corrigidos para o teste
mock_response = pd.DataFrame({
    "open": [150.00, 149.50],
    "high": [155.00, 150.00],
    "low": [149.00, 148.50],
    "close": [154.00, 149.00],  # Último valor ajustado para 149.00
    "volume": [2000, 1500]
})

# Testando carregar_dados_alpha_vantage com Mock
@patch('main.carregar_dados_alpha_vantage')
def test_carregar_dados_alpha_vantage(mock_carregar_dados):
    ...
    # Garantir que o último valor seja aproximadamente igual (evitando diferenças de precisão)
    assert abs(dados["close"].iloc[-1] - 149.00) < 1e-6, f"O último valor de fechamento deve ser 149.00, mas foi {dados['close'].iloc[-1]}."
